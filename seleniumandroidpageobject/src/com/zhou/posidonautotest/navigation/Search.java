...
public void testMediaSearch(){
	...
	if(false == total){
		TestngLog.log(url, "AssertSearch");
	}
...
public void testVideoSearch(){
	//搜索《社交网络》
	String url="http://ps.funshion.com/v5/search/video?q="+keywords+"&sz=10&cl=mweb&uc=30";
	boolean total= HttpAssert.AssertSearch(url, "videos", keywords2);
	if(false == total){
		TestngLog.log(url, "AssertSearch");
	}
